...
const DestinationContent: React.FunctionComponent<Props> = () => {
  ...
  const { data: allDestinations, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['all-destinations-homepage'],
    queryFn: () => {
      console.log('🎯 React Query: Starting fetch all destinations...')
  ...
  // Get 8 featured destinations
  const destinations = React.useMemo(() => {
    console.log('🔄 Computing featured destinations...')
    
    if (!allDestinations || allDestinations.length === 0) {
      console.log('⚠️ No destinations available')
      return []
    }
    
    console.log('📦 Total destinations:', allDestinations.length)
    
    // Ambil 8 destination pertama
    const result = allDestinations.slice(0, 8)
    console.log('🎯 Featured destinations (top 8):', result.length)
        ...
        <Box sx={{ 
          ...
          <Alert severity="error" sx={{ mb: 2 }}>
            <Typography variant="h6" sx={{ mb: 1 }}>
              ⚠️ Gagal memuat destinasi
            </Typography>
            <Typography variant="body2">
              {error instanceof Error ? error.message : 'Terjadi kesalahan saat memuat data destinasi'}
            </Typography>
          </Alert>
